...
for e=1:EE
    ...
    M=5;%number of antennas 用户数超过天线数量会跑不出数据
    Rth=5;%QoS for IUs
    ...
    PAF_relay=db2pow(40)*1e-3;%40DBm 10w
    Lmax=20;
    epsilon=1e-2;
    parfor i=1:LL
        ...
        %% -----------------OPTIMIZATION----------------------------
        [objA,iteA,flagA]=schemeA(L,K,M,N,PBS,PI,Rth,Eth,sigmaI,sigmaE, ...
            deltaI,Amax,lamda,PDyn,PSta,PBSCir,epsilon,Lmax,R,gr,gd,hr,hd);
        eeA(i,e)=objA(iteA+flagA);
        disp(['A:',num2str(eeA(i,e)),',i=',num2str(i),',e=',num2str(e),',iter=',num2str(iteA)]);
        [objB,iteB,flagB]=schemeB(L,K,M,N,PBS,Rth,Eth,sigmaI,sigmaE,lamda, ...
            PDyn,PSta,PBSCir,epsilon,Lmax,R,gr,gd,hr,hd);
        eeB(i,e)=objB(iteB+flagB);
        disp(['B:',num2str(eeB(i,e)),',i=',num2str(i),',e=',num2str(e),',iter=',num2str(iteB)]);
        [objC,flagC]=schemeC(L,K,M,PBS,Rth,Eth,sigmaI,sigmaE,lamda,PSta, ...
            PBSCir,epsilon,Lmax,gd,hd);
        ...
        [objE,iteE,flagE]=schemeE(L,K,M,N,PBS,PI,Rth,Eth,sigmaI,sigmaE,deltaI,Amax,lamda,PAF_relay,epsilon,Lmax,R,gr,gd,hr,hd);
