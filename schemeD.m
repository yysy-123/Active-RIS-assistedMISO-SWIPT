% passive RIS with random phase
function [obj,ite,flag]=schemeD(L,K,M,N,PBS,Rth,Eth, ...
    sigmaI,sigmaE,lamda,PDyn,PSta,PBSCir,epsilon,Lmax,R,gr,gd,hr,hd)
        ...
        jjjj=sqrt(-1);
        for phaseSet=1:N
            initial_phase = 2 * pi * rand;  % 连续随机相位
            v_p(phaseSet) = exp(jjjj * initial_phase);
        end
        v=v_a.*v_p;% 整个v
        flag=0;
        while 1
            ...
            while 1
                ...
                expression E2(1,L)
                for l=1:L
                    E2(l)=real(2*real(v_'*(S(:,:,l)+U(:,:,l))*(v_a.*v_p))-real(v_'*(S(:,:,l)+U(:,:,l))*v_)+(v_a.*v_p)'*(s(:,l)+u(:,l))+(s(:,l)'+u(:,l)')*(v_a.*v_p)+s_(l)+u_(l)+sigmaE);
                end
                ...
                for k=1:K
                    a1(k)-a2(k)>=Rth;
                    log(b1(k))>=log(2)*a1(k);
                    lamda*(2*real(v_'*(T(:,:,k))*(v_a.*v_p))-real(v_'*(T(:,:,k))*v_)+real((v_a.*v_p)'*t(:,k)+t(:,k)'*(v_a.*v_p)+d(k)))+1>=b1(k);
                    lamda*(quad_form((v_a.*v_p),T_(:,:,k))+real((v_a.*v_p)'*t_(:,k)+t_(:,k)'*(v_a.*v_p)+d_(k)))+1<=b2(k);
